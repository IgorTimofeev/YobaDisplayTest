...
void Driver::begin() {
	...
	spi_bus_config_t buscfg = {
		...
		.quadwp_io_num = -1,
		.quadhd_io_num = -1,
		.max_transfer_sz = _transactionScanlines * _display->getWidth() * 2 + 8
	};
...
void Driver::SPIPreTransferCallback(spi_transaction_t *transaction) {
...
void Driver::pushTransactions(int ypos) {
	...
	transactions[1].tx_data[2] = (_display->getWidth() - 1) >> 8;   //End Col High
	transactions[1].tx_data[3] = (_display->getWidth() - 1) & 0xff; //End Col Low
	...
	transactions[5].length = _display->getWidth() * _transactionScanlines * 2 * 8;  //Data length, in bits
...
uint8_t Driver::getTransactionScanlines() const {
